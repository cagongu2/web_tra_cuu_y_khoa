...
export const Favicon = () => {
  ...
  const {
    ...
    refetch,
  } = useGetImageByTypeQuery("favicon");

  const [uploadImage, { isLoading: isUploading }] = useUploadImageMutation();

  useEffect(() => {
    if (favicon && favicon.url) {
      setPreviewImage(getImgUrl(favicon.url));
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      // Kiểm tra kích thước file (tối đa 2MB - favicon thường nhỏ hơn)
      if (file.size > 2 * 1024 * 1024) {
        setNotification({
          type: "error",
          message: "Kích thước favicon không được vượt quá 2MB!",
        });
        return;
      }
      ...
      if (!validTypes.includes(file.type)) {
        setNotification({
          type: "error",
          message: "Chỉ chấp nhận file favicon (ICO, PNG, SVG)!",
        });
        return;
      }
  ...
  const onSubmit = async (data) => {
    try {
      if (!data.file) {
        setNotification({
          type: "warning",
          message: "Vui lòng chọn ảnh favicon trước khi tải lên!",
        });
        return;
      }
      ...
      setNotification({
        type: "error",
        message: "Đã xảy ra lỗi khi tải favicon lên.",
      });
    }
  };
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">
                        Tải lên và quản lý favicon cho website của bạn
                      </p>
                    </div>
                  </div>
                </div>

                {/* Comparison Box - 2 cột */}
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                  <div className="border-b border-gray-200 bg-gray-50 px-6 py-4">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                          So sánh favicon hiện tại và favicon mới trước khi cập nhật
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <span className="text-sm">Chọn ảnh favicon</span>
                        </label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 divide-x-0 md:divide-x divide-y md:divide-y-0 divide-gray-200">
                    ...
                    <div className="p-6">
                      ...
                      <div className="bg-gray-50 rounded-lg p-6 min-h-[200px] flex items-center justify-center border-2 border-green-200">
                        {favicon && favicon.url ? (
                          <div className="text-center space-y-4 w-full">
                            <div className="bg-white rounded-lg p-4 shadow-sm inline-block border border-gray-200">
                              <img
                                src={getImgUrl(favicon.url)}
                            ...
                            <div className="space-y-2">
                              <p className="text-sm font-medium text-gray-700">
                                Favicon đang sử dụng
                              </p>
                              <p className="text-xs text-gray-500">
                                Được hiển thị trên tab trình duyệt
                              </p>
                            </div>
                          </div>
                        ) : (
                          <div className="text-center space-y-3 text-gray-400">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-xs">Tải lên favicon đầu tiên</p>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="p-6">
                      ...
                      <div
                        className={`rounded-lg p-6 min-h-[200px] flex items-center justify-center border-2 ${
                          previewImage && !isServerImage
                      ...
                      {previewImage && !isServerImage && (
                        <div className="mt-4 bg-blue-50 rounded-lg p-3 border border-blue-200">
                          <div className="flex items-center space-x-2 text-blue-800">
                            ...
                            <span className="text-sm font-medium">
                              Favicon mới đã sẵn sàng để cập nhật
                            </span>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                      <div className="flex-1">
                        <p className="text-sm text-gray-600">
                          {previewImage && !isServerImage
                            ? "Kiểm tra kỹ favicon mới trước khi cập nhật"
                            : "Chọn favicon mới để bắt đầu so sánh"}
                        </p>
                      </div>
                      <div className="flex-shrink-0">
                        <button
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-700 mt-1 list-disc list-inside space-y-1">
                <li>Kích thước favicon tối đa: 2MB</li>
                <li>Định dạng hỗ trợ: ICO, PNG, SVG</li>
                <li>Kích thước khuyến nghị: 16x16px, 32x32px hoặc 64x64px</li>
                <li>Nên sử dụng định dạng ICO để tương thích tốt nhất</li>
                <li>Favicon sẽ hiển thị trên tab trình duyệt và bookmark</li>
              </ul>
            </div>
          </div>
        </div>
